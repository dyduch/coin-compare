...
class ArimaMethod:
                ...
                for q in range(self.max_q):
                    try:
                        ...
                        if new_rmse < best_rmse:
                            ...
                            best_rmse = new_rmse
                            print("New best rmse: {0} for parameters (p={1}, d={2}, q={3})".format(best_rmse, p, d, q))
                    except:
    ...
    def compute_model_and_rmse(self, train_df: pd.DataFrame, test_df: pd.DataFrame, p: int, d: int, q: int) -> (pd.DataFrame, float):
        test_start = test_df.iloc[0].name
        test_end = test_df.iloc[len(test_df.values) - 1].name
